...
const LodgingEditForm = () => {
  const currentLodging = useAppSelector((state) => state.lodging.current);
  const [name, setName] = useState<string>(currentLodging.name || "");
  const [description, setDescription] = useState<string>(
    currentLodging.description || ""
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await lodgingApi.update(currentLodging._id, formData);
      iziToast.success({
        message: `Гостиница "${name}" успешно обновлена`,
        position: "bottomCenter",
      });
      ...
      iziToast.error({
        message: err.message || "Ошибка при обновлении гостиницы",
        position: "bottomCenter",
      });
    }
  };
